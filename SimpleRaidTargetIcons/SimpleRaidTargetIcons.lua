...
local srti = {};
srti.version = C_AddOns.GetAddOnMetadata("simpleraidtargeticons","version");
...
local igMainMenuOptionCheckBoxOn = SOUNDKIT and SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON or 856 -- luacheck: ignore 211 (used in commented-out code)
local igMainMenuOptionCheckBoxOff = SOUNDKIT and SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_OFF or 857
local igMiniMapZoomOut = SOUNDKIT and SOUNDKIT.IG_MINIMAP_ZOOM_OUT or 824
local igMiniMapZoomIn = SOUNDKIT and SOUNDKIT.IG_MINIMAP_ZOOM_IN or 823
...
srti.frame = CreateFrame("Frame", "SRTIRadialMenu", UIParent);
...
function srti.frame:Show()
	...
	local clearLevel = srti.frame:GetFrameLevel() + 1;
	local iconLevel = clearLevel + 1;
	for i=1, 8 do
		local btn = CreateFrame("Button", "SRTIRadialIcon"..i, srti.frame, "SecureActionButtonTemplate");
		btn:SetAttribute("type1", "macro");
		btn:SetAttribute("macrotext1", "/tm "..i);
		btn:SetAttribute("type2", "macro");
		btn:SetAttribute("macrotext2", "/tm 0");
		btn:RegisterForClicks("AnyDown", "AnyUp");
		btn:SetFrameLevel(iconLevel);
		btn.tex = btn:CreateTexture(nil, "OVERLAY");
		btn.tex:SetAllPoints();
		btn.iconIndex = i;
		srti.frame[i] = btn;
	end
	srti.frame.clearBtn = CreateFrame("Button", "SRTIRadialClear", srti.frame, "SecureActionButtonTemplate");
	srti.frame.clearBtn:SetAttribute("type2", "macro");
	srti.frame.clearBtn:SetAttribute("macrotext2", "/tm 0");
	srti.frame.clearBtn:RegisterForClicks("AnyDown", "AnyUp");
	srti.frame.clearBtn:SetAllPoints();
	srti.frame.clearBtn:SetFrameLevel(clearLevel);
	...
	srti.frame:SetScript("OnUpdate", 
		function(self, arg1)
			...
			self.index = nil;
			-- GetRaidTargetIndex("target") returns a tainted secret value in 12.0.1.
			-- Reading it taints the OnUpdate execution context, breaking all comparisons.
			-- Use a clean test index in test mode only; skip checkmark in normal gameplay.
			local index = self.test and (srti.menu.test.index or 0) or nil;
...
--]]		                end
				...
				for i=1, 8 do
					...
					if ( index and index == i ) then
						t.tex:SetTexCoord(0,1,0,1);
						t.tex:SetTexture("Interface\\Buttons\\UI-GroupLoot-Pass-Up");
					else
						-- check 3rd party modifications first
						local PugLax = C_AddOns.IsAddOnLoaded("PugLax")
						if (PugLax and _G["PugLax"].CreateRaidIcon) and SRTISaved.PugLax then
							_G["PugLax"]:CreateRaidIcon(t.tex,i)
						else
							t.tex:SetTexture("Interface\\TargetingFrame\\UI-RaidTargetingIcons");
							SetRaidTargetIconTexture(t.tex,i);
						end
					end
				end
					...
					if ( self.index and ( not saved or saved == self.index ) ) then
						...
						if ( self.hovering > ( SRTISaved.hovertime or 0.2 ) ) then
							if not InCombatLockdown() then
								self[self.index]:Click("LeftButton", true);
							end
						end
					else
						self.hovering = 0;
	...
	-- PostClick handlers for icon buttons (secure /tm macros handle the actual SetRaidTarget)
	for i=1, 8 do
		srti.frame[i]:SetScript("PostClick", function(self, button)
			local parent = srti.frame;
			if parent.test then
				if button == "RightButton" then
					...
					srti.menu.test.index = self.iconIndex;
					SetRaidTargetIconTexture(srti.menu.test.icon, self.iconIndex);
				end
			end
			-- postClickPending is set by hooksecurefunc path when our macro fires.
			-- If not set and already hiding, the menu is being dismissed by something
			-- else; don't double-play sounds.
			if parent.hiding and not parent.postClickPending then return end
			parent.postClickPending = nil;
			if button == "RightButton" then
				parent.index = nil;
				PlaySound(igMiniMapZoomOut, "SPX");
			else
				parent.index = self.iconIndex;
				PlaySound(igMiniMapZoomIn, "SPX");
			end
			parent.showing = nil;
			parent.hiding = GetTime();
		end);
	end
	-- PostClick for clear overlay (right-click or left-click on empty space)
	srti.frame.clearBtn:SetScript("PostClick", function(self, button)
		local parent = srti.frame;
		if parent.test and button == "RightButton" then
			srti.menu.test.icon:Hide();
			srti.menu.test.index = nil;
		end
		if parent.hiding and not parent.postClickPending then return end
		parent.postClickPending = nil;
		if button == "RightButton" then
			local testIndex = parent.test and srti.menu.test.index;
...
-- SetRaidTarget is a protected function in 12.0.1 — cannot call it or any alias from
-- insecure Lua. Use a helper SecureActionButton to fire /tm macros instead.
srti.slashBtn = CreateFrame("Button", "SRTISlashButton", UIParent, "SecureActionButtonTemplate");
srti.slashBtn:SetAttribute("type", "macro");
srti.slashBtn:RegisterForClicks("AnyDown", "AnyUp");
srti.slashBtn:Hide();

-- Combat marking ring: pre-positioned buttons repositioned at cursor via restricted Lua.
-- Completely independent from the out-of-combat radial menu above.
-- 8 SecureActionButtonTemplate children fire /tm macros; custom state driver
-- repositions ring at cursor and shows/hides on combat + modifier.

srti.combatRing = CreateFrame("Frame", "SRTICombatRing", UIParent, "SecureHandlerStateTemplate");
srti.combatRing:SetSize(1, 1);
srti.combatRing:SetPoint("CENTER");
srti.combatRing:SetFrameStrata("DIALOG");

for i = 1, 8 do
	local btn = CreateFrame("Button", "SRTICombatBtn"..i, srti.combatRing, "SecureActionButtonTemplate");
	btn:SetSize(24, 24);
	btn:SetAttribute("*type1", "macro");
	btn:SetAttribute("*macrotext1", "/tm "..i);
	btn:SetAttribute("*type2", "macro");
	...
	-- Pre-position in a circle relative to parent center
	local radians = (0.375 - i / 8) * 360;
	btn:SetPoint("CENTER", srti.combatRing, "CENTER", 36 * cos(radians), 36 * sin(radians));
end

-- Update visibility state driver and scale from saved settings.
-- Called from UpdateSaved, ModiferCB, ScaleSlider.
function srti.UpdateCombatRing()
	if not SRTISaved then return end
	-- Build condition from enabled modifiers
	local parts = {};
	if SRTISaved.ctrl then parts[#parts + 1] = "[combat,mod:ctrl] show" end
	if SRTISaved.alt then parts[#parts + 1] = "[combat,mod:alt] show" end
	if SRTISaved.shift then parts[#parts + 1] = "[combat,mod:shift] show" end
	if #parts > 0 then
		parts[#parts + 1] = "hide";
		RegisterStateDriver(srti.combatRing, "visibility", table.concat(parts, "; "));
	else
		UnregisterStateDriver(srti.combatRing, "visibility");
		srti.combatRing:Hide();
	end
	srti.combatRing:SetScale(SRTISaved.radialscale or 1.0);
end

function srti.SetRaidTarget(index,unit,fromBinding)
	index = index or 0;
	-- In test mode, only handle updates from non-binding (clean) calls.
	-- fromBinding index is tainted; test mode updates are handled by PostClick.
	if ( srti.frame.test and not fromBinding ) then
		if ( index == 0 ) then
			srti.menu.test.icon:Hide();
			srti.menu.test.index = nil;
		else
	...
	if ( fromBinding ) then
		-- index is a tainted hook argument — don't store, compare, or use for sound.
		-- Signal PostClick to handle sounds and clean index tracking.
		if ( srti.frame:IsVisible() and not srti.frame.hiding ) then
			srti.frame.postClickPending = true;
			srti.frame.showing = nil;
			srti.frame.hiding = GetTime();
		end
	elseif ( srti.frame:IsVisible() and not srti.frame.hiding ) then
		-- index is a clean slash-command value; safe to compare.
	...
	if ( not fromBinding ) then
		if InCombatLockdown() then
			srti.Print("|cffff0000Cannot set raid target from slash commands during combat.|r");
		else
			srti.slashBtn:SetAttribute("macrotext", "/tm " .. index);
			srti.slashBtn:Click("LeftButton", true);
		end
	end
end
...
function srti.AddExternalFrameScripts()
	SRTIExternalUF = SRTIExternalUF or {}
	for _, frames in pairs(SRTIExternalUF) do
...
function srti.RegisterExternalFrame(frameName, addon, prebuilt)
	-- validate
	frameName = frameName or (GetMouseFoci and GetMouseFoci()[1] or GetMouseFocus()):GetName()
	...
	if addon and C_AddOns.IsAddOnLoaded(strlower(addon)) then
	...
	if not addon then
		...
		StaticPopupDialogs["SRTI_GET_ADDON_NAME"] = StaticPopupDialogs["SRTI_GET_ADDON_NAME"] or {
			...
			OnAccept = function(self,data)
				...
				if addon and not C_AddOns.IsAddOnLoaded(strlower(addon)) then
...
function srti.Options()
	...
	elseif Settings and Settings.RegisterCanvasLayoutCategory then
		local category = Settings.RegisterCanvasLayoutCategory(srti.menu, "SRTI")
		category.ID = "SRTI"
...
--	srti.menu.test:SetMovable(1);
	...
	--king srti.menu.test.model:SetLight(1,0,-.5,-.2,-.6,0.5,1,1,1,1,1,1,0.8);
	...
	if next(thirdParty) then
		srti.menu.Modifer3RDCB = function(self)
			SRTISaved[self.option] = not not self:GetChecked();
		end;
	...
	srti.menu.Update = function()
		...
		if next(thirdParty) then
			for _,addonname in pairs(thirdParty) do
	...
	function srti.keybindings.OnKeyDown(key)
		...
		elseif ( not srti.keybindings.mode and key ~= "LeftButton" and key ~= "RightButton" and key ~= "SHIFT" and key ~= "ALT" and key ~= "CTRL" and key ~= "UNKNOWN" ) then
			...
			if ( oldAction ~= "" and oldAction ~= srti.keybindings.binding ) then
				...
				if ( oldkeys > 1 ) then
					srti.keybindings.warntext:SetText(format(SRTI_BINDINGS_BIND_WARN_KEY, GetBindingText(oldAction, "BINDING_NAME_")));
				else
